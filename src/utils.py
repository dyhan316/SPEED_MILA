...
def split_raw(raw, window_length=60, keep_remainder = False):
    ...
    while start_sample < raw.n_times:
        ...
        if end_sample > raw.n_times:
            if keep_remainder:
                # Extend the last window to include the remainder
    ...
    #append the last window to the 2nd last window and remove the last window (also time_slices)
    if keep_remainder:
        #for some reason, mne.concatenate_raws does not work (separated, does not properly concatenated) 
        updated_raw = mne.io.RawArray(np.hstack([windows[-2]._data, windows[-1]._data]), windows[-2].info, verbose=False)
        updated_raw.set_montage(windows[-2].get_montage())
        windows[-2] = updated_raw
        time_slices[-2] = (time_slices[-2][0], time_slices[-1][1])
        del windows[-1]
        del time_slices[-1]
