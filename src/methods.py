...
class PreprocessMethods:
    ...
    def evaluate_quality(raw, oha_threshold=40e-6, thv_threshold=40e-6, chv_threshold=80e-6,
        ...
        thv = np.mean(np.std(raw._data, axis=0) > thv_threshold) #threshold variance
        chv = np.mean(np.std(raw._data, axis=1) > chv_threshold) #channel variance (% of channels with high variance)

        bad_channels = discrete_channels + noisychannels.get_bads()
        bcr = len(bad_channels) / n_chans #proportion of bad channels, if less than 0.8 then good
    ...
    def interpolate_nearest(raw, sfreq=256.0):
        ...
        with raw.info._unlock():
            ...
            # raw.info["lowpass"] = min(lowpass, sfreq / 2.0) #! interpolate does not do lowpass filtering (i.e. anti-aliasing) by default
            ...
            for line_freq in line_freqs:
                if line_freq < nyquist:
                    if lp_freq is not None and line_freq < lp_freq:
                        ...
                        if (preproc_line_noise_option is None) or (preproc_line_noise_option == False) : 
                            continue
                        elif preproc_line_noise_option == "default":
                            raw._data = dss_line_iter(raw._data.T, line_freq, sfreq)[0].T
                        elif preproc_line_noise_option == "mnenotch":
    ...
    def drop_extra_chs(raw, chs):
        extra_ch = [c for c in raw.ch_names if c not in chs]
        raw.drop_channels(extra_ch)
        return extra_ch
    
    def reorder_chs(raw, chs):
        new_ch_order = [ch for ch in chs if ch in raw.ch_names]    
        raw.reorder_channels(new_ch_order)    
        return new_ch_order